...
export default function SearchClient({
  ...
  const filteredProducts = useMemo(() => {
    const baseProducts = isUsingFallback ? SEARCH_PRODUCTS : shopifyProducts;
    return baseProducts.filter((product) => {
      // Query filter
      const matchesQuery =
        isUsingFallback && debouncedQuery
          ? product.title
              .toLowerCase()
  ...
  }, [debouncedQuery, activeFilters, shopifyProducts, isUsingFallback]);
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {(isUsingFallback ? filteredProducts.slice(0, visibleCount) : filteredProducts).map((product) => {
            ...
            return (
              <article
                key={product.id}
                className="group bg-surface-container-lowest rounded-xl border border-outline-variant/30 overflow-hidden flex flex-col hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"
              >
                <Link
